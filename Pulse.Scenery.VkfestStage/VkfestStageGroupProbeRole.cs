...
{
    public class VkfestStageGroupProbeRole : DecisionTreeRole
    {
        public VkfestStageGroupProbeRole(AbstractPulseAgent agent) : base
            (
                agent,
...
//            (_agent.MovementSystem as ISFMovementSystem).SetAgentForce(_agent, 20);
            ...
            var isGroupMoverequired = new FuncConditionBtNode<DecisionTreeData>(() =>
            {
                var plannedIntention = _agent.Role.Data.CurrentIntention as IPoiCurrentIntention;
                if (plannedIntention == null) return false;

                var movingGroup = Group as MovingAgentGroup;
                var leaderFormation = movingGroup.Formation as LeaderFormation;

                //is leader
                if (this == leaderFormation.Leader.Role) return false;

                return leaderFormation.Agents.Contains(this._agent);
            }, ad);

            var followLeaderMoveAction = new FuncBtNode<DecisionTreeData>(() =>
            {
                var movingGroup = Group as MovingAgentGroup;
                var leaderFormation = movingGroup.Formation as LeaderFormation;

                if (leaderFormation.Leader == null) return BehaviorState.Failed;
                if (_agent.MovementSystem == null) return BehaviorState.Failed;
                if (!_agent.MovementSystem.QueryVisibility(_agent.Point, leaderFormation.Leader.Point, _agent.Level, 0.1f)) return BehaviorState.Failed;

                var d = 5;
                _agent.DesiredPosition = leaderFormation.Leader.Point.DistanceTo(_agent.Point) > d ? leaderFormation.Leader.Point : _agent.Point;
            ...
            var groupMove = new SequenceBtNode();
            ...
            navigate.Children.Add(groupMove);
            ...
            groupMove.Children.Add(isGroupMoverequired);
            groupMove.Children.Add(followLeaderMoveAction);
